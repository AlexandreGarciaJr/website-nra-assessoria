    ...
    if (srv) {
      ...
      for (let opt of sel.options) { if (opt.value === srv) { sel.value = srv; break; } }
    }
    ...
    form.addEventListener('submit', async e => {
      ...
      try {
        ...
        if (json.status === 'ok') { msgOk.style.display = 'block'; form.reset(); }
        else throw new Error(json.message || 'Erro');
      } catch {
        msgErr.textContent = 'Ocorreu um erro ao enviar. Tente novamente ou entre em contato pelo WhatsApp.';
        msgErr.style.display = 'block';
      } finally {
        submitBtn.innerHTML = 'Enviar Solicitação <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z"/></svg>';
